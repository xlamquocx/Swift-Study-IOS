...
#import "TransmitterVC.h"

@interface TransmitterVC ()
...
- (void)viewDidLoad {
    ...
    self.beaconRegion = [[CLBeaconRegion alloc] initWithProximityUUID:uid major:1 minor:2 identifier:HM_SERVICE_ID];
...
- (IBAction)ibaSwitch:(id)sender {
    ...
    if (switchButton.on) {
        //Turn on
        [self turnOnBeacon];
        [ibEstimote setAlpha:1.0];
    } else {
        //Turn off
        [self turnOffBeacon];
        [ibEstimote setAlpha:0.3];
    }
}
@end
